...
function renderSignals() {
  ...
  list.innerHTML = sorted.map(s => {
    ...
    return '<div class="signal-card" onclick="openChart(\'' + s.symbol + '\')">' +
      ...
      '</div>' +
      '<div>Price: $' + (s.price ? s.price.toFixed(2) : '0.00') + '</div>' +
      '<div class="confidence">Confidence: ' + s.confidence + '% (' + (s.aligned || 0) + '/' + (s.totalStrategies || 10) + ' strategies)</div>' +
      (s.pattern ? '<div class="info">Pattern: ' + s.pattern + '</div>' : '') +
      (s.rsi ? '<div class="info">RSI: ' + s.rsi.toFixed(1) + ' | MACD: ' + s.macd.toFixed(4) + '</div>' : '') +
      '<div class="info">SL: $' + (s.stopLoss ? s.stopLoss.toFixed(2) : '0.00') + ' | TP: $' + (s.takeProfit ? s.takeProfit.toFixed(2) : '0.00') + '</div>' +
    ...
    list.innerHTML = reversed.map(s => {
      if (s.type === 'meme_coin') {
        return '<div class="history-card">' +
          ...
          '<div>Price: $' + (s.price ? s.price.toFixed(6) : '0') + ' | Prob: ' + s.probability + '%</div>' +
        '</div>';
      }
      const isBuy = s.direction === 'BUY';
      const color = isBuy ? '#00e676' : '#ff5252';
      return '<div class="history-card">' +
        '<div class="pair-row">' +
          ...
          '<span class="timeframe">' + s.timeframe + '</span>' +
        '</div>' +
        '<div>Price: $' + (s.price ? s.price.toFixed(2) : '0.00') + ' | Confidence: ' + s.confidence + '% (' + (s.aligned || 0) + '/' + (s.totalStrategies || 10) + ' strategies)</div>' +
        (s.pattern ? '<div class="info">Pattern: ' + s.pattern + '</div>' : '') +
        '<div class="info">SL: $' + (s.stopLoss ? s.stopLoss.toFixed(2) : '0.00') + ' | TP: $' + (s.takeProfit ? s.takeProfit.toFixed(2) : '0.00') + '</div>' +
