// Multi-strategy consensus engine (RSI, MACD, EMA crossover, ADX, volume)
// Confidence = % of active strategies that agree
...
function generateConsensusSignal(candles, currentPrice, rsi, macdHistogram, volumeSpike, priceVsMa) {
  ...
  // 4. ADX + DI direction (only if trend is strong enough)
  ...
  const totalVotes = votes.filter(v => v !== 0).length;
  ...
  // Confidence = percentage of active strategies that agree (0-100)
  const confidence = Math.round((maxVotes / totalVotes) * 100);

  // Require at least 4 active strategies and 75% agreement (or 3 active and 66% agreement)
  if (totalVotes < 3 || confidence < 66) return null;

  const direction = buyVotes > sellVotes ? 'BUY' : 'SELL';
  return {
    direction,
    confidence,                       // e.g., 75, 100
